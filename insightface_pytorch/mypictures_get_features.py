...
def process_all_image(folder_path):
    ...
    if "test" in files[0]:    #test 字符串出现在文件名里，就是测试集
        ...
        np.save('./data/save_mypic_test_fea', test_fea)       #把标签和特征存下来
        np.save('./data/save_mypic_test_lab', test_lab)
    else:                   #否则就是训练集
        train_fea = np.array(features)
        train_lab = np.array(labels)
        np.save('./data/save_mypic_train_fea', train_fea)
        np.save('./data/save_mypic_train_lab', train_lab)



if __name__ == '__main__':
    print("**处理图片中**")
    folder_path = './data/mypic_test'
    test_pic = process_all_image(folder_path)
    print(type(test_pic))
    folder_path1 = './data/mypic_train'
    train_pic = process_all_image(folder_path1)
    print("**处理图片完成**")
    test_fea = np.load('./data/save_mypic_test_fea.npy')
    test_lab = np.load('./data/save_mypic_test_lab.npy')
    train_fea = np.load('./data/save_mypic_train_fea.npy')
    train_lab = np.load('./data/save_mypic_train_lab.npy')
